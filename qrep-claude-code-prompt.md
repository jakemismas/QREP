...
- A proportion lock toggle, default LOCKED. Locked: width and height move together by scaling cell size only; grid counts never change, so the pattern is preserved exactly. Unlocked: width and height move independently by adding or removing whole blocks (quantized — never fractional blocks), and the viewer displays the achieved dimensions next to the requested ones when they differ
- A width input per border band (mixed fractions), re-rendering live like the size controls
...
- The design reference docs/viewer-mock.html is expected in the repo (Jake generated it); match its visual intent while keeping the functional contract above. If it is absent, design from this spec and note that in the closing comment
...
- Emitted HTML contains ruler markup, the preset values, and the proportion lock control (string-asserted in tests)
- The sizing math — including locked cell-size scaling and unlocked whole-block quantization with achieved-vs-requested dimensions — is mirrored in a small pure-Python helper used by the emitter and unit-tested against the same numbers the JS uses (keeps JS honest without a browser test rig)
...
Stages, k-selection, identity-rectification path, confidence formulas, and accuracy definitions per design doc. CLI: `qrep reverse img.png -o recovered.json [--corners ...] [--fabrics N]` — escape hatches for real photos only; round-trip tests pass the image path alone. Also `qrep compare truth.json recovered.json`: prints grid dims match, cell accuracy, palette mapping, and per-stage confidence side by side (the harness comparison, exposed). Test images generated by the S6 renderer with fixed seeds at test time, never committed or hand-edited. Thresholds below are copied verbatim into test literals and never edited.
...
- `qrep compare` runs on a round-trip pair and its output reconciles with the harness numbers (tested)
...
Ship a testable release. Set version 0.1.0 in pyproject. Build distributions (`.venv/Scripts/python -m pip install build` then `python -m build` for sdist + wheel). Generate the demo artifact set fresh from the benchmark fixture: PDF booklet, cut list (md + csv), yardage report, top-diagram SVG, viewer.html, L0 and L2 renders, and the L0 recovered.json with its `qrep compare` output saved as compare.txt. Create the GitHub release: `gh release create v0.1.0 --title "QREP v0.1.0" --notes-file <notes>` attaching the wheel, sdist, and a zip of the demo artifacts. Release notes: what works, the three strategies' headline metrics, CV accuracy per level, known issues summary, and a 5-line quickstart.

Acceptance:
- `python -m build` produces sdist and wheel; the wheel installs into a fresh venv and `qrep --help` exits 0
- Every demo artifact regenerates fresh during this slice via CLI commands (listed in the closing comment)
- `gh release view v0.1.0` shows the release with all assets attached
- Release notes contain the quickstart, metrics, and a link to REPORT.md
- pytest and ruff green; CI green on main for S9

## Final quality bar

A quilter with the generated PDF and no other context could cut and assemble the benchmark quilt — and a quilter with the viewer HTML could resize it confidently. Jake can download the v0.1.0 release in the morning and test everything without touching the repo. Everything upstream serves that.
